...
- **数据链路就绪**：才允许取数。
- 工作目录：若 cwd 不是 vault（__VAULT_ROOT__），提示用户新建会话时选择 vault 目录；复盘笔记写入 `{{cwd}}/复盘/`，不依赖 cwd。
...
通用复盘模板文件：`__PROJECT_ROOT__/templates/review-template.md`（每日市场复盘 / 个股复盘通用，含市场概况、涨停梯队、板块热点、龙虎榜、消息面、次日关注、风险提示，用 `{{...}}` 占位）。

生成复盘笔记时：**先用 `read` 读取该模板**，按当天数据填充每个 `{{...}}` 占位，输出到 `{{cwd}}/复盘/YYYY-MM-DD.md`；数据不可用的占位如实标注"暂无"，不编造。
