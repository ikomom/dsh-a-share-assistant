...
## 持仓股分析（position review）

把台账持仓与**当日真实行情**逐只对照，判定"触发了没、该加仓/持有/止盈/止损"。判定引擎照《复盘模板生成指南》§3，**只标注「计划 vs 现实」的偏差，不换股、不改计划参数**。
...
node __PROJECT_ROOT__/src/cli.js position review [--date D] [--account X]     # 分析（默认：终端表格 + Markdown 片段 + HTML 报告）
...
- **三种输出**：① 终端表格（分组 + 操作取向）② Markdown 片段（直接贴进复盘笔记，**模板不动**）③ 单文件 HTML 报告（默认 `{{cwd}}/复盘/持仓分析[-账户]-YYYY-MM-DD.html`，涨红跌绿、ECharts 已内联，**断网也能出图**）。
- **口径**：股票用**未复权**价判止损/目标（前复权会和当初的计划价错位）；ETF 走 fund 接口（只有前复权）。快照取不到自动退回最近一根日线并标明数据源；仍取不到则按成本价并**显式告警**（浮盈是假象）。
