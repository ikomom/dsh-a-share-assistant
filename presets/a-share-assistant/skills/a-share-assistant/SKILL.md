...
生成复盘笔记时：**先用 `read` 读取模板**，按当天数据填充；输出到 `{{cwd}}/复盘/YYYY-MM-DD.md`。
...
**输出位置（约定，别散放）**：HTML 报告默认写 `{{cwd}}/复盘/持仓分析/持仓分析[-账户]-YYYY-MM-DD.html`——与当日复盘笔记 `{{cwd}}/复盘/YYYY-MM-DD.md` **同一层下的按类型子目录**，复盘根目录只留笔记和固定文件夹。引用报告时用相对路径 `复盘/持仓分析/持仓分析-YYYY-MM-DD.html`。
