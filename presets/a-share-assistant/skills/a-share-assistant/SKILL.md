...
7. **省 token**：`data` 默认只输出摘要（共 N 条 + 前 5 条），需要全部加 `--full`；取大数据建议 `--save` 落盘后单条查询/`grep`，**不要全文读入上下文**；缓存/取数结果优先落盘并在需要看具体值时用 `cache latest` 或 grep 定位。

## 复盘模板

模板文件：`__PROJECT_ROOT__/templates/review-template.md`（每日复盘模板：大盘环境 / 主线热点 / 涨停数据 / 龙虎榜 / 操作回顾 / 交易心理 / 认知增量 / 明日计划——只含有数据源（fuyao）或由用户填写的板块，资金面等无数据源的已剔除）。

生成复盘笔记时：**先用 `read` 读取模板**，按当天数据填充。

**AI 只填"数据可查"板块**：一~四（大盘环境、主线热点、涨停数据、龙虎榜）——用 fuyao 数据 + 网络检索填充。
**个人主观板块留空**：五~八（操作回顾、交易心理、认知增量、明日计划+铁律）——这些是用户自己的交易记录/判断，**AI 不臆造、不替写**，标注"请用户填写"或留空。
